...
function last_n_corpus_and_predictions (state, pred_i, opts) {
...
function reducer (opts) {
  return function (state, action) {
    ...
    case 'select-prediction':
      ...
      let ngram = last_n_corpus_and_predictions(state, pred_i, opts)
      // console.log('selected', state.selected_predictions)
      // console.log(ngrm, 'ngrm')
      // produce new prediction
      let new_pred = state.nextF(ngram)
      // add it to list of predictions
      state.predictions.push(new_pred)
      return state
...
module.exports = function (modelStreamF, opts) {
  ...
  modelS.onError(err  => {
    store.dispatch({ type: 'error', error: err.toString() })
  })
  modelS.onValue(model => {
    store.dispatch({ type: 'model', model: model          })
    store.dispatch({ type: 'error', error: null           }) // clear error
  })
  return store
}
